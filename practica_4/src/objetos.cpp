...
void _triangulos3D::calcular_normal_caras() {
...
void _triangulos3D::calcular_normal_vertices() {
  if (normales_caras.size() != caras.size()) calcular_normal_caras();
...
void _triangulos3D::draw_iluminacion_suave() {
  if (normales_vertices.size() != vertices.size()) calcular_normal_vertices();
...
void _brazoRobot::MueveBase(float alfa) {
  this->alfaBase = fmod((this->alfaBase + alfa), 360.0);
}

void _brazoRobot::MueveBrazo(float alfa) {
  this->alfaBrazo = fmod((this->alfaBrazo + alfa), 360.0);
}

void _brazoRobot::MueveAntebrazo(float alfa) {
